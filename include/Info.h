...
* получен набор понятий.
* Строка символов может быть *любой*, она не разделена на языки, т.к. Info -
* это просто поток. Мы не называем это "данными", чтобы подчеркнуть: поток
* содержит то, что может быть *воспринято* персонажем.
*/
class Info {
public:
